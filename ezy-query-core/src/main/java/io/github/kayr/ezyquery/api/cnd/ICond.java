...
public interface ICond {
    ...
    default Optional<Conds> combine(ICond cond, BinaryExpr.Op combineOperator) {
        ...
        if (isConds() && cond.isConds() &&
              thisOp == otherOp &&
              combineOperator == thisOp) {

            List<Object> cnds1 = ((Conds) this).getCnds();
            List<Object> cnds2 = ((Conds) cond).getCnds();

            return Optional.of(Conds.createConds(((Conds) cond).getOperator(), Elf.combine(cnds1, cnds2)));

        }

        if (isConds() &&
             !cond.isConds() &&
              thisOp == combineOperator) {
